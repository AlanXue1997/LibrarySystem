			...
			public void actionPerformed(ActionEvent e) {
				if (userName.getText().equals("")) {
					...
					if (String.valueOf(passWord.getPassword()).equals(LoginFrame.this.libraryDatabase.getPasswd(userName.getText()))) {
						Person person = LoginFrame.this.libraryDatabase.getPerson(userName.getText());
						if(person.getIdType() == Person.STUDENT){
							Frame frame = new Frame(LoginFrame.this.libraryDatabase, person);
						}
						else{
							ManagerFrame managerFrame = new ManagerFrame(LoginFrame.this.libraryDatabase, person);
							managerFrame.frame.setVisible(true);
						}
						dispose();
						// new MainFrame();
					} 
